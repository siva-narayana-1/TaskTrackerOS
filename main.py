...
from token_generator import load_token, remove_token, view_logged, remove_locked_user
        ...
        if choice == "1":
            ...
            if session:
                ...
                if role == "admin" and check_token.get(username) == token:
                    ...
                    print(f"{username} was Locked. Ask Admin to unlock.")
                    continue 
                else:
...
def admin_dashboard(role, username, token):
    ...
    while True:
        check_token = load_token()
        choice = input("1. View Users\n2. Remove Locked User.\n3. Logout\nChoose an option: ")
        if choice == "1" and check_token.get(username) == token:
            view_logged(username)
        elif choice == "2" and check_token[username] == token:
            remove_locked_user(username,role)
        elif choice == "3" and check_token.get(username) == token:
...
def user_dashboard(role, username, token):
    ...
    while True:
        ...
        if choice == "1" and check_token.get(username) == token:
            add(username)
        elif choice == "2" and check_token.get(username) == token:
            view(username)
        elif choice == "3":
...
if __name__ == "__main__":
    main()
>>>>>>> f1f0fbfe736b9e85060971a7d57bce82b3ccfd20
